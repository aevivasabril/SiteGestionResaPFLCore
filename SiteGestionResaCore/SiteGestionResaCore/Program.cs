        ...
        public static async Task Main(string[] args)
        {
            ...
            using(var scope = hostReturn.Services.CreateScope())
            {
                using (var db = scope.ServiceProvider.GetRequiredService<GestionResaContext>())
                {
                    ...
                    {
                        var usr = new utilisateur { Email = options.Value.email, UserName = options.Value.email, nom = options.Value.nom, prenom = options.Value.prenom, EmailConfirmed = true, organismeID = 1, equipeID = 6, accordDonnees = true };
